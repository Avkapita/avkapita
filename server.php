...
$mail->Username = ''; // почта, с которой шлем письмо
$mail->Password = '';  // пароль от почты
$mail->SMTPSecure = 'ssl';  // тип шифрования
$mail->Port = 465;  // TCP порт (смотрится на сайте почты)

//Recipients
$mail->setFrom('', 'Компания вкусных бургеров!'); // почта, с которой шлем письмо. !Обязательно совпадает с почтой выше
$mail->addAddress('');     // почта получателя (должна брвться из формы)
